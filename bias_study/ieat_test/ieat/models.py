...
from ieat.clip_models import DebiasedCLIP
from ieat.clip_models import CLIP_prompt
from ieat.clip_models import CLIP_clipped
from ieat.clip_models import CLIP_prompt_age

def load_json(sent_file):
    ''' Load from json. We expect a certain format later, so do some post processing '''
    logging.info("Loading %s..." % sent_file)
...
class EmbeddingExtractor:
	...
	def extract_dir(self, test_name, text_type, d, file_types=(".jpg", ".jpeg", ".png", ".webp"), batch_size=None, visualize=False, **extract_params):
		"""
	...
	def extract(self, test_name, text_type, image_paths, batch_size=None, output_path=None, gpu=False, visualize=False, **extract_kwargs):
		"""
		...
		with torch.no_grad():  # saves some memory
			if text_type == None:
				...
				TEST_EXT = '.jsonl'
				text_data_dir = '/home/h/haoyu/CLIP_debiasing/validation_exp/sent-bias/tests_aligned'
				texts = load_json(os.path.join(text_data_dir, "%s%s" % (test_name, TEST_EXT)))[text_type]['examples']
				...
				encs = pd.concat([
					pd.DataFrame(
						self._extract_context_text(self.process_samples_text(batch, visualize=visualize), gpu, **extract_kwargs)
					)
					for batch in batches_texts
				])

				encs["img"] = [text for text in texts]
	...
	def process_samples_text(self, image_paths, visualize=False):
		"""
		Pre-process the image samples for embedding extraction.
	...
	def _extract_context_text(self, samples, gpu, **extract_kwargs) -> np.ndarray:
		raise NotImplementedError
...
class CLIPExtractor(EmbeddingExtractor):
	...
	def __init__(self, model_name, n_px, **parent_params):
		"""
		...
		# self.test_model = DebiasedCLIP("ViT-B/16", device='cpu')
		# self.test_model.load_state_dict(torch.load("../vl_debiasing/exp/fyp_19/debiased_CLIP_weights_fyp_19_10epochs.pth"))
		# self.test_model = CLIP_clipped("ViT-B/16", device='cpu', hidden_dim=512, m=256)
		self.test_model = CLIP_prompt_age("ViT-B/16", device='cpu')

	def _extract_context(self, samples, gpu, **extract_kwargs) -> np.ndarray: 
		output = self.model_output(samples, gpu)
		return output.numpy() if not gpu else output.cpu().numpy()

	def _extract_context_text(self, samples, gpu, **extract_kwargs) -> np.ndarray: # todo
		output = self.model_output_text(samples, gpu)
		return output.numpy() if not gpu else output.cpu().numpy()
	...
	def load_model(self):
		...
		'''
		self.model, self.preprocessor = clip.load(version, device='cpu')
		# print(self.model)
	...
	def process_samples(self, image_paths, visualize=False):
		...
		# dim=(n_px,n_px)
		samples = np.zeros((len(image_paths), 3, self.n_px, self.n_px),dtype=np.float64)
		# print("Correct shape", samples.shape)
		for n,image_path in enumerate(image_paths):
			img = Image.open(image_path)
			# print(self.preprocessor(img).unsqueeze(0).shape)
			img_np = np.array(self.preprocessor(img).unsqueeze(0))
			# print(img_np.shape)
			samples[n] = img_np
		return torch.from_numpy(samples).type(torch.LongTensor)
	...
	def process_samples_text(self, texts, visualize=False): # todo
			
		# dim=(n_px,n_px)
		samples = np.zeros((len(texts), 77),dtype=np.float64)
		# print("Correct shape", samples.shape)
		for n,text in enumerate(texts):
			tokenized = clip.tokenize(text, truncate = True)
			text_np = np.array(tokenized)
			# print(img_np.shape)
			samples[n] = text_np
		# print(samples.shape)
		return torch.from_numpy(samples).type(torch.LongTensor)
	...
	def model_output(self, samples, gpu):
		...
		"""
		return self.model.encode_image(samples) if self.test_model == None else self.test_model.encode_image(samples)

	def model_output_text(self, samples, gpu): # todo
		"""
		Model output from every layer for a given input image.
		Embeddings can be extracted and aggregated from different layers (see the child classes).
		...
		output : tuple(torch.FloatTensor)
			a Tensor of all hidden states
		"""
		return self.model.encode_text(samples) if self.test_model == None else self.test_model.encode_text(samples)
