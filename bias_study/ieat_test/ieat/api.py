from ieat.models import SENTExtractor, OpenAIExtractor, LogitExtractor, SimCLRExtractor, CLIPExtractor
...
def test(
	...
	**test_params
	...
	text_type = ['targ1', 'targ2', 'attr1', 'attr2']
	...
	for i in range(len(input_dirs)):
		if modality[i] == 'image':
			logger.progress(f"Extracting images from {input_dirs[i]}")
			embeddings.append(extractor.extract_dir(
				None,
				None,
				input_dirs[i], file_types,
				visualize=verbose,
				gpu=gpu,
				batch_size=batch_size
			))
		else:
			logger.progress(f"Extracting texts from {input_dirs[i]}")
			embeddings.append(extractor.extract_dir(
				test_name,
				text_type[i],
				None, file_types,
...
def test_all(
	...
	for model_type, model_params in model_types.items():
		...
		for test_data in to_test:
			...
			categories.append(test_data.name)
