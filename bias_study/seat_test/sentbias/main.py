...
def handle_arguments(arguments):
    ...
    parser.add_argument('--debiased', action='store_true',
                        help='Apply debiaser for text debiasing.')
...
def main(arguments):
    ...
    for model_name in models:
        ...
        if model_name == ModelName.BOW.value:
            ...
            model_options = ''
        elif model_name == ModelName.CLIP.value:
            model_options = ''
        else:
        ...
        for test in tests:
            ...
            if not args.ignore_cached_encs and os.path.isfile(enc_file):
                ...
                if model_name == ModelName.BOW.value:
                    ...
                    if args.debiased:
                        debiaser = MLP(512, 512, 2)
                        debiaser.load_state_dict(torch.load("../vl_debiasing/debiaser_best.pth"))
                        model = clip.load_model()
                        encs_targ1 = clip.encode(model, encs["targ1"]["examples"], debiaser=debiaser)
                        encs_targ2 = clip.encode(model, encs["targ2"]["examples"], debiaser=debiaser)
                        encs_attr1 = clip.encode(model, encs["attr1"]["examples"], debiaser=debiaser)
                        encs_attr2 = clip.encode(model, encs["attr2"]["examples"], debiaser=debiaser)
                    else:
                        model = clip.load_model()
                        encs_targ1 = clip.encode(model, encs["targ1"]["examples"])
                        encs_targ2 = clip.encode(model, encs["targ2"]["examples"])
                        encs_attr1 = clip.encode(model, encs["attr1"]["examples"])
                        encs_attr2 = clip.encode(model, encs["attr2"]["examples"])
