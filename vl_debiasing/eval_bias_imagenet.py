...
def run_eval(model, preprocess, dataset='fairface', attribute="gender"):
    model.eval()

    # def eval(model):

    # measure bias, lower == less biased
    bias_result = clip_debiasing.measure_bias(model, preprocess, clip.tokenize, attribute=attribute, dataset=dataset)
    print(bias_result)
...
if __name__ == '__main__':
    ...
    # model.load_state_dict(torch.load("./exp/fyp_2/debiased_CLIP_weights_fyp_2_70epochs.pth"))
    ...
    model = CLIP_clipped("ViT-B/16", device='cuda', attribute='age', hidden_dim=512, m=400)
    preprocess = model.preprocess

    # model = CLIP_prompt_age("ViT-B/16", device='cuda')
    # preprocess = model.preprocess
