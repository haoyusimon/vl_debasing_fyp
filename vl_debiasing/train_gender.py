...
from clip_debiasing.datasets import FairFaceDebiasing
import clip
...
def train(model, data_loader, optimizer, epoch=None, warmup_steps=None, device=None, scheduler=None, config=None):
    ...
    for i, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_ita, text_sim_loss, text_diff_loss = model(image, text1, text2, word1, word2, epoch)
...
def main(args=None, config=None):
    ...
    shutil.copy("./clip_debiasing/models/model_contrastive_queue_pareto.py", os.path.join(work_dir, 'model_details.py'))
    shutil.copy("./train_simple.py", os.path.join(work_dir, 'train_details.py'))
    ...
    dataset = FairFaceDebiasing(tokenizer=clip.tokenize, transforms=model.preprocess)
    ...
    for epoch in range(epochs):
        ...
        if epoch % 2 == 0 or (epoch + 1) % 10 == 0:
            ...
            eval_train.run_eval_train(model, model.preprocess)
            sys.stdout.flush()  
            sys.stderr.flush()
